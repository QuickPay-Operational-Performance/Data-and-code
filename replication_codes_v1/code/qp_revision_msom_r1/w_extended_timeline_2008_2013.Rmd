---
title: "QuickPay Delay Rate: Extended Timeline"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    keep_tex: false
...
```{r folder_for_raw_files,include=FALSE}
...
# For 2008-2012, using the files specific to agency code 97
# For 2013, have all files for all agenecies (so subsetting)
...
```{r select_timeline,include=FALSE}
# We are interested in this timeline
...
reg_df[,delay:=fifelse(contract_award_unique_key==lag(contract_award_unique_key),
                       ...
                       NaN)]
reg_df[,wins_delay:=Winsorize(delay,val=quantile(delay,probs=c(0.025,0.975),na.rm=T))]
reg_df[,wins_initial_duration:=Winsorize(initial_duration,val=quantile(initial_duration,probs=c(0.05,0.95),na.rm=T))]
reg_df[,wins_initial_budget:=Winsorize(initial_budget,val=quantile(initial_budget,probs=c(0.05,0.95),na.rm=T))]
reg_df[,wins_offers:=Winsorize(number_of_offers_received,val=quantile(number_of_offers_received,probs=c(0.05,0.95),na.rm=T))]
reg_df[,relative_delay:=delay/(1+initial_duration)]
reg_df[,wins_relative_delay:=Winsorize(relative_delay,val=quantile(relative_delay,probs=c(0.025,0.975),na.rm=T))]
...
reg_df[,wins_project_quarter_stage:=100*Winsorize(project_quarter_stage,  
                                                  val=quantile(project_quarter_stage,probs = c(0.05, 0.95),na.rm=T))]
```
