...
for(df in df_list){
  print(i)
  setDT(df)
  df[,wins_percentage_delay:=100*Winsorize(percentage_delay,val=quantile(percentage_delay,probs=c(0.025,0.975),na.rm=T))]
  df[,winsorized_delay:=Winsorize(delay,val=quantile(delay,probs=c(0.025,0.975),na.rm=T))]
