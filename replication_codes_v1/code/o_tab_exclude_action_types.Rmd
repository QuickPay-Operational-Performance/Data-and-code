---
title: "Excluding certain action types"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    keep_tex: false
...
```{r regs2, include=FALSE, warning=FALSE}
m0=feols(wins_percentage_delay~treat_new+
                            post_t:treat_new+
                            log(wins_project_quarter_stage)+
                            log(1+winsorized_initial_duration_in_days_i)+
...
etable(exclude_scope_action_types,
       ...
       digits='r2',
       digits.stats = 2,
       caption="Excluding projects with certain action types",
       headers=list(" "=list("Exclude action types D, A, L, and G"=as.numeric(length(exclude_scope_action_types)),
                    "Exclude all action types except M"=as.numeric(length(exclude_other_action_types)))),
