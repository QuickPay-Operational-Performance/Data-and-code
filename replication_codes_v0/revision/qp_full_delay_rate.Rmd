...
```{r fp_contracts, include=FALSE,warning=FALSE,message=FALSE}
...
```{r cost_contracts, include=FALSE,warning=FALSE,message=FALSE}
m1=feols(wins_percentage_delay~treat_new+
        post_t:treat_new+
          post_t,
      cluster=~contract_award_unique_key,
...
```{r model_free_plot, include=FALSE}
...
reg_df[, time_to_treat := ifelse(treat_new==1, relative_quarter, 0)]
...
#  geom_line(alpha=0.7,size=0.25)+
  ...
  ylab("Point estimate \n (Percent delay)")#+ 
  #ylim(-1.05, 2.5)+
  # scale_y_continuous(breaks = seq(min(data_event_study$estimate),
  #                                 max(data_event_study$estimate), 
  #                                 0.5))
```