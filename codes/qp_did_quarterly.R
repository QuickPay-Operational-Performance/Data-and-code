...
#### Firms with one type of contract: Performance-based contracts with FAO terciles #####
...
pba_dict=unique(df_raw, by = "contract_award_unique_key")
pba_df=merge(df3,pba_dict,by= "contract_award_unique_key")

# performance based
no_fe_pba_tercile_1=felm(winsorized_delay ~ after_quickpay*small_business |
                 0|0|0,     
               data = subset(pba_df,performance_based_service_acquisition_code=='Y'&
                               fao_weight_tercile==1))

no_fe_pba_tercile_2=felm(winsorized_delay ~ after_quickpay*small_business |
                           0|0|0,     
                         data = subset(pba_df,performance_based_service_acquisition_code=='Y'&
                                         fao_weight_tercile==2))

no_fe_pba_tercile_3=felm(winsorized_delay ~ after_quickpay*small_business |
                           0|0|0,     
                         data = subset(pba_df,performance_based_service_acquisition_code=='Y'&
...
no_fe_non_pba_tercile_1=felm(winsorized_delay ~ after_quickpay*small_business |
                           0|0|0,     
                         data = subset(pba_df,performance_based_service_acquisition_code!='Y'&
                                         fao_weight_tercile==1))

no_fe_non_pba_tercile_2=felm(winsorized_delay ~ after_quickpay*small_business |
                           0|0|0,     
                         data = subset(pba_df,performance_based_service_acquisition_code!='Y'&
                                         fao_weight_tercile==2))

no_fe_non_pba_tercile_3=felm(winsorized_delay ~ after_quickpay*small_business |
                           0|0|0,     
                         data = subset(pba_df,performance_based_service_acquisition_code!='Y'&
                                         fao_weight_tercile==3))


stargazer(no_fe_non_pba_tercile_1,no_fe_non_pba_tercile_2,no_fe_non_pba_tercile_3,
          title = "Days of Delay (Winsorized): Quickpay Dec 2009- June 2012",
          dep.var.labels.include = TRUE,
          object.names=FALSE, 
          model.numbers=FALSE,
