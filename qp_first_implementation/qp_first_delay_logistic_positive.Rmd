...
baseline_reg=feglm(positive_delay~treat_i+
                   post_t:treat_i+
                   post_t,
                 data=reg_df,
                 family='binomial',
...
controls_and_no_fe=feglm(positive_delay~treat_i+
                           post_t:treat_i+
                           post_t+
...
# Logistic Regressions (Positive Delay) -- One type

```{r baseline_regressions_one_type, echo=FALSE, results='asis',warning=FALSE}
# Baseline Regressions 
...
etable(baseline_reg,
       ...
       extralines=list("Controls"=c("", rep("Yes",4)),
                       "Controls $\\times Post_t$"=c("", rep("Yes",4))),
       file=paste0(tables_folder,"/base_positive_logistic_one_type.tex"))
