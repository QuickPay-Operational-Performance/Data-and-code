---
title: "Percentage Delay Rate (Three Quarters): QuickPay (2009-2012)"
...
num_quarters=3
# determine delay over num_quarters
df[,delay:=ifelse(contract_award_unique_key==lag(contract_award_unique_key,num_quarters), 
                  last_reported_end_date-lag(last_reported_end_date,num_quarters),NaN)]
...
ggplot(mean_delay, aes(x=year_quarter,
    ...
    annotate("text", label = "QuickPay\n implemented", x = 6, y = 30,color="dimgrey")+
    geom_vline(xintercept = 8.2, linetype="solid", 
                color = "gray", size=1.5)+
...
```{r normalized_plot,echo=FALSE, eval=FALSE}

# the time periods are not very standardized since we are looking at delays over two quarters without resampling again 
...
ggplot(mean_delay, aes(x=year_quarter,
    ...
    scale_x_discrete(limits=mean_delay$year_quarter)+
    geom_point(size=3)+
    labs(x="Year-Quarter", y = "Average percentage delay rate per two quarters \n (normalized by delay in 2010-Jun) ")+  
    annotate("text", label = "QuickPay\n implemented", x = 8, y = 160,color="dimgrey")+
...
# Baseline Regressions (Truncated Sample with Positive Delays)
...
baseline_reg=felm(wins_percentage_delay~treat_i+
                    ...
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=subset(reg_df, wins_percentage_delay>0),
                  exactDOF = TRUE, 
                  cmethod = "reghdfe")
# time fixed effects also included in the following specs
controls_and_no_fe=felm(wins_percentage_delay~treat_i+
