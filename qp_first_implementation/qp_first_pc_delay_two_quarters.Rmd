...
LB_1Q<- sort(na.omit(reg_df$delay))[round(length(na.omit(reg_df$delay))*0.025)+1]
UB_1Q<- sort(na.omit(reg_df$delay))[round(length(na.omit(reg_df$delay))*0.975)+1]
```
...
baseline_reg=felm(delay~treat_i+
                    ...
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=subset(reg_df,delay>LB_1Q & delay<UB_1Q), 
                  exactDOF = TRUE, 
                  cmethod = "reghdfe")
# time fixed effects also included in the following specs
...
LB_2Q_nz<- sort(na.omit(df_non_zero_2Q$two_quarter_delay))[round(length(na.omit(df_non_zero_2Q$two_quarter_delayy))*0.025)+1]
