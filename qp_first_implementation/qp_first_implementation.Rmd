...
ggplot(mean_delay, aes(x=year_quarter,
    ...
    geom_point(size=3)+
    labs(x="Year-Quarter", y = "Average delay rate per quarter (in days)")+  
    annotate("text", label = "QuickPay\n implemented", x = 8, y = 15,color="dimgrey")+
...
small_baseline=subset(mean_delay,business_type=="S" & year_quarter=="2010-Mar")$V1
large_baseline=subset(mean_delay,business_type=="O" & year_quarter=="2010-Mar")$V1
...
setnames(mean_stats,c('winsorized_delay',
                     ...
                     'mean_number_of_offers_received'))
...
setnames(num_unique_stats,c("naics_code",
                            "product_or_service_code",
                            "contract_award_unique_key"),
                     c('number_of_firms',
                     'number_of_tasks',
...
path='~/Desktop/research/Git:Github/QuickPay/qp_data_and_code/qp_first_implementation/'
...
```{r fao, include=FALSE}
# read raw data
df_raw=fread('~/Dropbox/data_quickpay/qp_data/qp_data_fy10_to_fy18.csv')
...
intellect_data=fread('~/Dropbox/data_quickpay/qp_data/govt_weight_per_recipient.csv',select=c('recipient_duns','sales_fiscal_year','sales_volume'))
...
all_fe_and_controls_reg=felm(winsorized_delay~treat_i+
                    ...
                    post_t:winsorized_initial_budget_i+
                    post_t:number_of_offers_received|
                    product_or_service_code+action_date_year_quarter+naics_code| # no fixed effects
                    0| # no IV
                  contract_award_unique_key, # clustered at project level
                  data=reg_df,
                  exactDOF = TRUE,
...
<!-- ![](~/Desktop/cts_te.png){width=10%,height=10%} -->
...
all_fe_and_controls_reg=felm(winsorized_delay~low_if+
                    ...
                    post_t:winsorized_initial_budget_i+
                    post_t:number_of_offers_received|
                    naics_code+product_or_service_code+action_date_year_quarter| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df,
                  exactDOF = TRUE,
