...
**Hypothesis:** QuickPay made small projects more attractive.
  * Number of bids received for competitively awarded small projects increased. 
  * These new projects experienced greater delays.
...
```{r competition_delays, echo=FALSE, results='asis'}
...
## Non-competitively awarded New Projects: Impact on Delays

```{r non_competition_delays, echo=FALSE, results='asis'}
...
<!-- # Impact of Awarding and Funding Office -->

<!-- $$ FundingOffice_i = \begin{cases} 1, \text{ if project } i \text{ had different funding and awarding offices before QuickPay}\\ -->
<!-- 0, \text{ otherwise} \end{cases}$$ -->

<!-- $$ \begin{aligned} -->
<!-- Delay_{it} &=& \alpha+\beta_0 Treat_i + \beta_1 Post_t + \beta_2 (Treat_i \times Post_t) \\ -->
<!-- &+&\beta_3 FundingOffice_i + \beta_4 (FundingOffice_i \times Post_t) + \beta_5 (Treat_i \times Post_t \times FundingOffice_i) \\  -->
<!-- &+&X_i + (Post_t \times X_i) + \mu_t + \theta_{firm} + \lambda_{task}+ \epsilon_{it} -->
<!-- \end{aligned}$$ -->

```{r funding_office, eval=F, echo=F, results='asis'}
both_projects = intersect(unique(subset(reg_df,post_t==1)$contract_award_unique_key),
                          unique(subset(reg_df,post_t==0)$contract_award_unique_key))

df_first_reported[, unique_awarding_office_code:=paste(awarding_sub_agency_code,awarding_office_code,sep="_")]
df_first_reported[, unique_funding_office_code:=paste(funding_sub_agency_code,funding_office_code,sep="_")]

df_first_reported[,different_office:=ifelse(unique_awarding_office_code!=unique_funding_office_code,1,0)]
df_first_reported[,same_office:=ifelse(unique_awarding_office_code==unique_funding_office_code,1,0)]

reg_df=merge(reg_df,
              df_first_reported[,c('contract_award_unique_key',
                                   'different_office',
                                   'same_office')],
              by='contract_award_unique_key',all.x=T)

baseline_reg=felm(winsorized_delay~treat_i+
                    post_t:treat_i+
                    post_t+
                    different_office+
                    post_t:treat_i:different_office|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=subset(reg_df,contract_award_unique_key%in%both_projects), 
                  exactDOF = TRUE, 
                  cmethod = "reghdfe")
...
controls_time_task_fe=felm(winsorized_delay~treat_i+
                            ...
                            winsorized_initial_budget_i+
                            number_of_offers_received|
                            post_t+product_or_service_code|
                            0|
                            contract_award_unique_key,
                          data=subset(reg_df,contract_award_unique_key%in%both_projects), 
                          exactDOF = TRUE, 
...
controls_and_all_fe=felm(winsorized_delay~treat_i+
                            ...
                            winsorized_initial_budget_i+
                            number_of_offers_received|
                            post_t+product_or_service_code+recipient_duns|
                            0|
                            contract_award_unique_key,
                          data=subset(reg_df,contract_award_unique_key%in%both_projects), 
                          exactDOF = TRUE, 
...
vars.order=c('treat_i',
             'post_t',
              'treat_i:post_t',
              'different_office',
              'treat_i:post_t:different_office')
...
stargazer(baseline_reg,
          ...
          digits=2,
          digits.extra=2,
          title = "Effect of Different Funding Office: Quickpay 2009-2011",
          dep.var.labels="$Delay_{it}$ (in days)",
          dep.var.caption = "",
          ...
          covariate.labels = c("$Treat_i$",
                               ...
                               "$Treat_i \\times Post_t$",
                               "$FundingOffice_i$",
                               "$Post_t \\times FundingOffice_i \\times Treat_i$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
          ...
          notes=c("Each observation is a project-quarter.",
                  "SEs are robust and clustered at the project level.",
                  "Sample restricted to projects that were active both before and after QuickPay."),
          header=F)
```
