...
![](/Users/vibhutidhingra/Desktop/research/Git:Github/qp_data_and_code/img/competition_1.png){width=5%,height=5%}
  
We, therefore, need the effect coming from projects that were signed after QuickPay. 

![](/Users/vibhutidhingra/Desktop/research/Git:Github/qp_data_and_code/img/competition_2.png){width=5%,height=5%}
...
controls_and_no_fe=felm(number_of_offers_received~treat_i+
                                  treat_i:post_t+
                                  post_t+
                            winsorized_initial_duration_in_days_i+
                            winsorized_initial_budget_i+
...
controls_and_time_fe=felm(number_of_offers_received~treat_i+
                                  treat_i:post_t+
                            winsorized_initial_duration_in_days_i+
                            winsorized_initial_budget_i+
                            post_t:winsorized_initial_duration_in_days_i+
...
controls_time_task_fe=felm(number_of_offers_received~treat_i+
                                  treat_i:post_t+
                            winsorized_initial_duration_in_days_i+
                            winsorized_initial_budget_i+
                            post_t:winsorized_initial_duration_in_days_i+
...
stargazer(baseline_reg,
          ...
          column.sep.width = "-2pt",
          add.lines = list(
                           c("Duration, Budget",rep("No",1),rep("Yes",3)),
                           c("$Post_t \\times $  (Duration, Budget)",rep("No",1),rep("Yes",3)),
...
stargazer(baseline_reg,
          ...
          covariate.labels = c("$Treat_i$",
                               "$SignedAfterQP_i$",
                               "$Post_t$",
                               "$Treat_i \\times SignedAfterQP_i \\times Post_t$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
stargazer(baseline_reg,
          ...
          covariate.labels = c("$Treat_i$",
                               ...
                               "$Treat_i \\times StartedAfterQP_i \\times Post_t$",
                               "$Treat_i \\times StartedAfterQP_i \\times Post_t \\times Competitive_i$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
```{r competition_delay_three way, echo = F, results='asis'}

reg_df[,competitive_and_started_after_QP:=ifelse(competitively_awarded_i==1 &
                                      project_signed_after_quickpay==1,1,0)]

baseline_reg=felm(winsorized_delay~treat_i+
                                  competitive_and_started_after_QP+
                                  treat_i:post_t+
                                  treat_i:post_t:competitive_and_started_after_QP+
                                  post_t|
                                  0| # no fixed effects
                                  0| # no IV
                                  contract_award_unique_key, # clustered at project level
...
stargazer(baseline_reg,
          ...
          digits=2,
          digits.extra=2,
          title = "Effect of Competition: Quickpay 2009-2011",
          dep.var.labels="$Delay_{it}$ (in days)",
          dep.var.caption = "",
          ...
          covariate.labels = c("$Treat_i$",
                               ...
                               "$Treat_i \\times Post_t$",
                               "$Artificial_i$",
                               "$Post_t \\times Artificial_i \\times Treat_i$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
### Projects started after QuickPay

```{r delays_plot_after_qp, echo=F, results='asis'}
mean_delay=reg_df[!is.na(winsorized_delay) & action_date_year_quarter<as.Date('2012-06-30'),
                  mean(winsorized_delay),
                  by = c('action_date_year_quarter',
                         'project_signed_after_quickpay')]

mean_delay[,year_quarter:=format(action_date_year_quarter,"%Y-%b")]
mean_delay[,project_signed_after_quickpay:=as.factor(project_signed_after_quickpay)]
mean_delay=mean_delay[order(action_date_year_quarter)]#,business_type)]

ggplot(mean_delay, aes(x=year_quarter,
                                   y=V1, 
                                   group = project_signed_after_quickpay,
                                   shape=project_signed_after_quickpay, 
                                   linetype=project_signed_after_quickpay))+
  geom_line() +    
  scale_x_discrete(limits=mean_delay$year_quarter)+
  geom_point(size=3)+
  labs(x="Year-Quarter", y = "Average delay per quarter (in days)")+  
  annotate("text", label = "QuickPay\n implemented", x = 4, y = 50,color="dimgrey")+
  geom_vline(xintercept = 5.5, linetype="solid", 
             color = "gray", size=1.5)+
  scale_shape_discrete(name  ="Project started\n after QP",
                       breaks=c("1", "0"),
                       labels=c("Yes", "No"))+
  scale_linetype_discrete(name  ="Project started\n after QP",
                          breaks=c("1", "0"),
                          labels=c("Yes", "No"))+
  theme(axis.text.x = element_text(angle = 90),
        axis.line = element_line(),
...
### Effect of Competition for Small and Large Projects

```{r delays_plot_competition_and_size, echo=F, results='asis'}
...
mean_delay[,year_quarter:=format(action_date_year_quarter,"%Y-%b")]
mean_delay[,competitively_awarded_i:=as.factor(competitively_awarded_i)]
mean_delay=mean_delay[order(action_date_year_quarter,business_type)]

ggplot(subset(mean_delay,business_type=="S"), aes(x=year_quarter,
                                                  y=V1, 
                                                  group = competitively_awarded_i,
                                                  shape=competitively_awarded_i, 
                                                  linetype=competitively_awarded_i))+
  ggtitle("All Small Projects")+ # for the main title
  geom_line() +    
  scale_x_discrete(limits=mean_delay$year_quarter)+
  geom_point(size=3)+
  labs(x="Year-Quarter", y = "Average delay per quarter (in days)")+  
  annotate("text", label = "QuickPay\n implemented", x = 15, y = 50,color="dimgrey")+
  geom_vline(xintercept = 18.5, linetype="solid", 
             color = "gray", size=1.5)+
  scale_shape_discrete(name  ="competitively awarded",
                       breaks=c("1", "0"),
                       labels=c("Yes", "No"))+
  scale_linetype_discrete(name="competitively awarded",
                          breaks=c("1", "0"),
                          labels=c("Yes", "No"))+
...
ggplot(subset(mean_delay,business_type=="O"), aes(x=year_quarter,
                                                  y=V1, 
                                                  group = competitively_awarded_i,
                                                  shape=competitively_awarded_i, 
                                                  linetype=competitively_awarded_i))+
  ggtitle("All Large Projects")+ # for the main title
  geom_line() +    
  scale_x_discrete(limits=mean_delay$year_quarter)+
  geom_point(size=3)+
  labs(x="Year-Quarter", y = "Average delay per quarter (in days)")+  
