...
stargazer(baseline_reg,
          ...
          dep.var.labels="$Delay_{it}$ (in days)",
          dep.var.caption = "",
          covariate.labels = c("$Treat_i$","$Post_t$","$Treat_i$x$Post_t$","Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          digits=2,
          digits.extra=2,
          ...
          add.lines = list(c("Year-Quarter Fixed Effects","No","Yes","Yes"),
                           ...
                           c("$Post_t$  x  (Duration, Budget, Bids)","No","Yes","Yes")),
...
stargazer(baseline_reg,
          ...
          covariate.labels = c("$Treat_i$",
                               ...
                               "$Treat_i$x$Post_t$",
                               "$FinancialAid$",
                               "$Post_t$ x $FinancialAid$",
                               "$Post_t$ x $FinancialAid$ x $Treat_i$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
# We don't have sales data for all firms in intellect 
# Some sales are 0 so (1+sales)
small_projects_fao[, rho:=ifelse(sales_fiscal_year=="2010 Sales Volume",
                                 sum_fao/(1+sales_volume),NaN)] 

# get empirical distribution of rho, defined by alpha
small_projects_fao=small_projects_fao[order(rho)]
small_projects_fao[,alpha:=ifelse(sales_fiscal_year=="2010 Sales Volume",seq.int(nrow(small_projects_fao))/nrow(small_projects_fao),NaN)]

small_projects_fao=small_projects_fao[,c('recipient_duns','alpha')]
```

```{r treat_intensity_regs, echo=FALSE, results='asis'}

reg_df=merge(reg_df,small_projects_fao,by='recipient_duns',all=T)
reg_df[,treat_intensity_i:=treat_i*alpha]

# Baseline Regressions 
baseline_reg=felm(winsorized_delay~treat_intensity_i+post_t:treat_intensity_i+post_t|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df, 
...
# time fixed effects also included in the following specs
controls_and_firm_fe=felm(winsorized_delay~treat_intensity_i+
...
controls_and_firm_task_fe=felm(winsorized_delay~treat_intensity_i+
                            post_t:treat_intensity_i+
                            winsorized_initial_duration_in_days_i+
                            winsorized_initial_budget_i+
                            number_of_offers_received+
