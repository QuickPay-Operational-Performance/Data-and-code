...
projects_per_quarter=reg_df[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(projects_per_quarter,'V1','num_projects')

small_projects_per_quarter=subset(reg_df,treat_i==1)[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(small_projects_per_quarter,'V1','num_small_projects')

large_projects_per_quarter=subset(reg_df,treat_i==0)[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(large_projects_per_quarter,'V1','num_large_projects')

contractors_per_quarter=reg_df[,n_distinct(recipient_duns),
                               by='action_date_year_quarter']
setnames(contractors_per_quarter,'V1','num_contractors')

projects_per_contractor=merge(projects_per_quarter,
                              contractors_per_quarter,
                              by='action_date_year_quarter')

projects_per_contractor=merge(projects_per_contractor,
                              small_projects_per_quarter,
                              by='action_date_year_quarter')

projects_per_contractor=merge(projects_per_contractor,
                              large_projects_per_quarter,
                              by='action_date_year_quarter')

projects_per_contractor[,projects_per_contractor:=num_projects/num_contractors]
projects_per_contractor[,small_projects_per_contractor:=num_small_projects/num_contractors]
projects_per_contractor[,large_projects_per_contractor:=num_large_projects/num_contractors]


projects_per_contractor <- melt(projects_per_contractor, 
                                id = "action_date_year_quarter", 
                                measure = c("projects_per_contractor",
                                            "small_projects_per_contractor",
                                            "large_projects_per_contractor"))

ggplot(projects_per_contractor,
       aes(x=as.factor(action_date_year_quarter),
           y=value,
           group=variable))+
geom_line(aes(linetype=variable),alpha=0.7)+
geom_point(aes(shape=variable),size=1.5)+
  theme_minimal()+
  labs(x="Time", 
       y = "Number of projects per contractor")+
  scale_linetype_manual(name  ="",
                        breaks=c("projects_per_contractor", 
                                 "small_projects_per_contractor",
                                 "large_projects_per_contractor"),
                        labels=c("All","Treated","Control"),
...
### Continuous version: Full Sample

```{r num_projects_0_regs,warning=FALSE, echo=FALSE, results='asis'}

projects_per_contractor_quarter=reg_df[,n_distinct(contract_award_unique_key),
                                       by=c('recipient_duns','action_date_year_quarter')]

setnames(projects_per_contractor_quarter,'V1','num_projects_held_by_contractor')

reg_df_0=merge(reg_df,projects_per_contractor_quarter,
               by=c('recipient_duns','action_date_year_quarter'))

m1=felm(wins_percentage_delay~treat_i+
                    post_t:treat_i+
                     post_t+
                    num_projects_held_by_contractor+
                    post_t:num_projects_held_by_contractor+
                    treat_i:num_projects_held_by_contractor+
                    post_t:treat_i:num_projects_held_by_contractor|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df_0,
...
num_projects_table=stargazer(m1,m2,m3,m4,m5,
          digits=2,
          digits.extra=2,
          ...
          covariate.labels =c("$Treat_i$",
                               "$Post_t$",
                               "Num Contractor Projects",
                               "$Treat_i \\times Post_t$",
                               "$Post_t \\times$ Num Contractor Projects",
                               "$Treat_i \\times$ Num Contractor Projects",
                               "$Treat_i \\times Post_t \\times$ Num Contractor Projects",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
## Discrete version: Full Sample

```{r num_projs_discrete, warning=FALSE,echo=FALSE,results='asis'}

reg_df_0[,num_projs_quintile:=ntile(num_projects_held_by_contractor,5)]

df_main=data.frame()
for(i in 1:length(unique(reg_df_0$num_projs_quintile))){
#  print(i)
  tryCatch({
  a=tidy(felm(wins_percentage_delay~treat_i+
                ...
                0|
                contract_award_unique_key,
              data=subset(reg_df_0,num_projs_quintile==i),
              exactDOF = TRUE, 
              cmethod = "reghdfe"),
              conf.int=T)
    a=subset(a,term=='treat_i:post_t')
    a['Num projects quintile']=i
    assign(paste0("df", i), value = a)
    df_main=rbind(df_main,get(paste0("df", i)))}, error=function(e){})
}

setDT(df_main)[,`Num projects quintile`:=as.factor(`Num projects quintile`)]
...
ggplot(df_main,
       aes(x=`Num projects quintile`,y=estimate,group=1))+
...
## Restricted Sample: Contractors active before QuickPay

```{r num_projects_1,warning=FALSE, echo=FALSE, results='asis'}

contractors_active_before_quickpay=unique(subset(reg_df,post_t==0)$recipient_duns)

projects_per_quarter=subset(reg_df,recipient_duns%in%contractors_active_before_quickpay)[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(projects_per_quarter,'V1','num_projects')

small_projects_per_quarter=subset(reg_df,recipient_duns%in%contractors_active_before_quickpay&
                                    treat_i==1)[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(small_projects_per_quarter,'V1','num_small_projects')

large_projects_per_quarter=subset(reg_df,recipient_duns%in%contractors_active_before_quickpay&
                                    treat_i==0)[,n_distinct(contract_award_unique_key),
                               by='action_date_year_quarter']
setnames(large_projects_per_quarter,'V1','num_large_projects')

contractors_per_quarter=subset(reg_df,recipient_duns%in%contractors_active_before_quickpay)[,n_distinct(recipient_duns),
                               by='action_date_year_quarter']
setnames(contractors_per_quarter,'V1','num_contractors')
...
geom_point(aes(shape=variable),size=1.5)+
  ...
  theme(axis.text.x = element_text(angle = 90),
        ...
        legend.position="bottom",
        legend.key.width=unit(2.5,"cm"))+
  ggtitle("Sample restricted to contractors active before QuickPay")
```

### Continuous version: Restricted Sample

```{r num_projects_1_regs,echo=FALSE, warning=FALSE, results='asis'}
m1=felm(wins_percentage_delay~treat_i+
                    post_t:treat_i+
                     post_t+
                    num_projects_held_by_contractor+
                    ...
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=subset(reg_df_0,recipient_duns%in%contractors_active_before_quickpay),
                  exactDOF = TRUE, 
                  cmethod = "reghdfe")
...
m3=felm(wins_percentage_delay~treat_i+
                           log(wins_project_quarter_stage)+
                            ...
                            0|
                            contract_award_unique_key,
                          data=subset(reg_df_0,
                                      recipient_duns%in%contractors_active_before_quickpay), 
                          exactDOF = TRUE, 
                          cmethod = "reghdfe")
# time fixed effects also included in the following specs
m4=felm(wins_percentage_delay~treat_i+
...
num_projects_table=stargazer(m1,m2,m3,m4,m5,
          digits=2,
          digits.extra=2,
          title = "Num Contractor Projects and QuickPay reform: Restricted Sample",
          dep.var.labels="$PercentDelay_{it}$  ",
          dep.var.caption = "",
          covariate.labels =c("$Treat_i$",
                               "$Post_t$",
          ...
          notes=c("Each observation is a project-quarter.",
                  "SEs are robust and clustered at the project level.",
                  "Sample restricted to contractors active before QuickPay."),
          header=F)
```

### Discrete version: Restricted Sample

```{r num_projs_discrete_restricted, warning=FALSE,echo=FALSE,results='asis'}

reg_df_01=subset(reg_df_0,recipient_duns%in%contractors_active_before_quickpay)

reg_df_01[,num_projs_quintile:=ntile(num_projects_held_by_contractor,5)]

df_main=data.frame()
for(i in 1:length(unique(reg_df_01$num_projs_quintile))){
#  print(i)
  tryCatch({
  a=tidy(felm(wins_percentage_delay~treat_i+
                treat_i:post_t+
                ...
                0|
                contract_award_unique_key,
              data=subset(reg_df_01,num_projs_quintile==i),
              exactDOF = TRUE, 
              cmethod = "reghdfe"),
              conf.int=T)
    a=subset(a,term=='treat_i:post_t')
...
```{r num_projects_2, warning=FALSE, eval=FALSE, echo=FALSE}

# Did contractors work on more projects after QuickPay?
# If yes, did those contractors have greater delays after the reform

num_projects=subset(reg_df,
                    recipient_duns%in%contractors_active_before_quickpay)[,n_distinct(contract_award_unique_key),
                    by=c('business_type','action_date_year_quarter')]

setnames(num_projects,'V1','num_projects')
num_projects[,year_quarter:=format(action_date_year_quarter,"%Y-%b")]
num_projects=num_projects[order(action_date_year_quarter,business_type)]

# this shows number of active projects in a given year quarter
# large businesses presumably have longer projects, so may have more active ones at any point in time
ggplot(num_projects, 
       aes(x=as.factor(action_date_year_quarter),
           y=num_projects,
           group = business_type))+
geom_line(aes(linetype=business_type, 
              color=business_type),
          alpha=0.7) +
geom_point(aes(shape=business_type),size=1.5)+
  theme_minimal()+
...
reg_df_num_projs=merge(reg_df,
             num_projects,
             by=c('business_type',
                  'action_date_year_quarter'))

reg_df_num_projs=subset(reg_df_num_projs,recipient_duns%in%contractors_active_before_quickpay)
...
m4=felm(num_projects~treat_i+
          post_t:treat_i|
          action_date_year_quarter+
          product_or_service_code+
          naics_code| # no fixed effects
          0| # no IV
          contract_award_unique_key, # clustered at project level
        data=reg_df_num_projs,
        exactDOF = TRUE,
...
stargazer(m1,m2,m3,m4,m5,
          digits=2,
          digits.extra=2,
          title = "Number of Projects after QuickPay",
          dep.var.labels="Number of projects$_{it}$  ",
          dep.var.caption = "",
          covariate.labels = c("$Treat_i$",
                               "$Post_t$",
                                "$Treat_i \\times Post_t$",
          ...
          omit.stat=c("f", "ser"),
          column.sep.width = "-2pt",
          add.lines = list(c("Time fixed effects",rep("No",1),rep("Yes",4)),
                           c("Task fixed effects",rep("No",2),rep("Yes",3)),
                           c("Industry fixed effects",rep("No",3),rep("Yes",2)),
...
portfolio_table=stargazer(m1,m2,m3,m4,m5,
          ...
          covariate.labels =c("$Treat_i$",
                               ...
                               "$Treat_i \\times Post_t \\times$ Proportion Large",
