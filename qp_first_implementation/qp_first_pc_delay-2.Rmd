...
## Stage Quintile Regression Plots

```{r stage_quintile, echo=FALSE,results='asis'}


reg_df[,stage_quintile:=as.factor(ntile(project_quarter_stage,5))]

# tidy(lm(wins_percentage_delay~treat_i*post_t*num_projects_tile,
#         data=reg_df))
...
m1=felm(wins_percentage_delay~treat_i+
            ...
            0|
            contract_award_unique_key,
          data=subset(reg_df,stage_quintile=="1"),
          exactDOF = TRUE, 
          cmethod = "reghdfe")
...
m2=felm(wins_percentage_delay~treat_i+
          ...
          0|
          contract_award_unique_key,
        data=subset(reg_df,stage_quintile=="2"),
        exactDOF = TRUE, 
        cmethod = "reghdfe")
...
m3=felm(wins_percentage_delay~treat_i+
             ...
             0|
             contract_award_unique_key,
           data=subset(reg_df,stage_quintile=="3"),
           exactDOF = TRUE, 
           cmethod = "reghdfe")
...
m4=felm(wins_percentage_delay~treat_i+
          ...
          0|
          contract_award_unique_key,
        data=subset(reg_df,stage_quintile=="4"),
        exactDOF = TRUE, 
        cmethod = "reghdfe")
...
m5=felm(wins_percentage_delay~treat_i+
          ...
          0|
          contract_award_unique_key,
        data=subset(reg_df,stage_quintile=="5" & 
                      project_quarter_stage<100),
        exactDOF = TRUE, 
        cmethod = "reghdfe")

data_to_plot=multiplot(m1,m2,m3,m4,m5,
          coefficients ="treat_i:post_t",
          plot=F)
data_to_plot=setDT(data_to_plot)
data_to_plot[,`Project stage quintile`:=c(1,2,3,4,5)]
data_to_plot[,`Project stage quintile`:=as.factor(`Project stage quintile`)]

ggplot(data_to_plot,aes(x=`Project stage quintile`,y=Value))+
  geom_point(size=2,alpha=0.7)+
  geom_errorbar(aes(ymin = LowInner, 
                    ymax = HighInner),
                width=0.05,
                alpha=0.5)+
theme_minimal()+ 
...
fixest::iplot(mod_twfe, 
      ...
      grid=F,
      ylim.add=0.20,
      zero.par=list(lwd=0.5),
      #ci.fill = T,
      pt.join = T,
      x=c(-5:4))

# f=fixest::iplot(mod_twfe,only.params=T,
#       x=c(-5:4))
# f$prms
```
