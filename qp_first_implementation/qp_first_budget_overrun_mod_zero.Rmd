---
title: "Budget Overruns (Modification Zero): QuickPay (2009-2012)"
...
df=fread(paste0(directory,'resampled_qp_data/qp_resampled_data_fy10_to_fy12.csv'))
...
df_first_reported[,period_of_performance_start_date:=as.Date(period_of_performance_start_date)]
...
# this suffices coz modification number is same for all observations of a given project 
# (because we looked at first reported info)

reg_df=subset(reg_df,modification_number=="0" & 
                period_of_performance_start_date<as.Date('2011-04-27'))
...
ggplot(mean_overrun, aes(x=year_quarter,
    ...
    annotate("text", label = "QuickPay\n implemented", x = 12, y = 8000,color="dimgrey")+
...
ggplot(mean_overrun, aes(x=year_quarter,
    ...
    scale_x_discrete(limits=mean_overrun$year_quarter)+
    geom_point(size=3)+
    labs(x="Year-Quarter", y = "Average overrun rate per quarter (in days)")+  
...
reg_df[,percentage_overrun:=ifelse(contract_award_unique_key==lag(contract_award_unique_key,1),
         ...
         NaN)]
reg_df[,wins_percentage_overrun:=Winsorize(100*percentage_overrun,na.rm=T)]
```
...
ggplot(mean_overrun, aes(x=year_quarter,
    ...
    geom_point(size=3)+
    labs(x="Year-Quarter", y = "Average overrun rate per quarter")+  
    annotate("text", label = "QuickPay\n implemented", x = 8, y = 30,color="dimgrey")+
    geom_vline(xintercept = 10.2, linetype="solid", 
                color = "gray", size=1.5)+
    scale_shape_discrete(name  ="Business Size",
                          breaks=c("O", "S"),
