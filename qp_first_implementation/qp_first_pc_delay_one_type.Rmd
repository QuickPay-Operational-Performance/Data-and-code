---
title: "Percentage Delay Rate (with one type of contractor): QuickPay (2009-2012)"
...
* Sample restricted to contractors holding only one type of project in the sample horizon.
* Except where we examine "Spillover effect" -- there we use a "clean control"
   * This means we keep all small projects
   * We keep only those large project-quarters that don't have a concurrent small project.
...
robustness_folder='~/Desktop/Research/QuickPay/paper/Tables/robustness'
...
base_table<-stargazer(baseline_reg,
          ...
          controls_time_task_fe,
          controls_and_all_fe,
          title = "Effect of QuickPay on days of delay rates",
...
# Write to file
write(base_table_tex,
      paste0(robustness_folder,"/base_table_delay_days_one_type.tex"))
```
...
treated_projs_preqp=subset(reg_df_one_type,treat_new==1 & post_t==0)
control_projs_preqp=subset(reg_df_one_type,treat_new==0 & post_t==0)
...
treated_projs_postqp=subset(reg_df_one_type,treat_new==1 & post_t==1)
control_projs_postqp=subset(reg_df_one_type,treat_new==0 & post_t==1)
...
## Impact on bidding metrics [One type]
...
reg_df_first_reported=subset(df_first_reported, recipient_duns%in%one_type_contractors &
                             action_date<=as.Date('2012-06-30')&
                               contract_award_unique_key%in%projects_to_keep$contract_award_unique_key)[,..select_cols_comp]
...
duration=felm(winsorized_initial_duration_in_days_i~treat_i+
                          ...
                          data=subset(reg_df_first_reported,
                                      competitively_awarded_i==1 &
                                        winsorized_initial_duration_in_days_i>=50), 
                          exactDOF = TRUE, 
                          cmethod = "reghdfe")
