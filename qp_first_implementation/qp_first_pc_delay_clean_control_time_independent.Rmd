...
# Summary statistics
# num_small_large_projects[,contractor_type:=case_when(num_large_projects==0 ~ "Only small",
#                                                      num_small_projects==0 ~ "Only large",
#                                                      num_large_projects>0 & num_small_projects>0 ~ "Both small and large")]
# 
# summary(subset(num_small_large_projects,contractor_type=='Only small')$num_small_projects)
# 
# summary(subset(num_small_large_projects,contractor_type=='Only large')$num_large_projects)
# 
# summary(subset(num_small_large_projects,contractor_type=='Both small and large')$num_small_projects)
# 
# summary(subset(num_small_large_projects,contractor_type=='Both small and large')$num_large_projects)

```

```{r get_budget_project_types, include=FALSE}
treat_before_qp=unique(setDT(subset(reg_df,treat_i==1 & post_t==0)), by='recipient_duns')
budget_small_projects=treat_before_qp[,sum(base_and_all_options_value),by='recipient_duns']
setnames(budget_small_projects,'V1','budget_small_projects')

control_before_qp=unique(setDT(subset(reg_df,treat_i==0 & post_t==0)), by='recipient_duns')
budget_large_projects=control_before_qp[,sum(base_and_all_options_value),by='recipient_duns']
setnames(budget_large_projects,'V1','budget_large_projects')

relative_budget=merge(budget_large_projects,
                      budget_small_projects,
                      by='recipient_duns',
                      all=T)
relative_budget[is.na(relative_budget)]=0
...
## Sample restricted to projects active before QP
```{r sample_before_qp,echo=FALSE,results='asis'}
started_before_qp=unique(subset(reg_df_subset,post_t==0)$contract_award_unique_key)
...
baseline_reg=felm(wins_percentage_delay~treat_new+
                    ...
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=subset(reg_df_subset, contract_award_unique_key%in%started_before_qp),
                  exactDOF = TRUE, 
                  cmethod = "reghdfe")
# time fixed effects also included in the following specs
controls_and_no_fe=felm(wins_percentage_delay~treat_new+
...
base_table<-stargazer(baseline_reg,
                      ...
                      notes=c("Each observation is a project-quarter.",
                              "SEs are robust and clustered at the project level.",
                              "Sample restricted to projects that started before QP."),
                      header=F)
```

# Treatment Intensity

* Treat_Intensity_i = Treat_i x log(Budget of project i)
* This measures how much a contractor is affected by QuickPay
* The interaction term roughly tells us the additional delay on a given project for every dollar that the contractor earns the small project.
* Sample restricted to projects that started before QP

```{r treat_intensity_budget, results='asis', echo=FALSE, warning=FALSE}
#reg_df_subset[,treat_intensity_i:=num_small_projects/(num_small_projects+num_large_projects)]
reg_df_subset[,treat_intensity_budget_i:=treat_new*log(winsorized_initial_budget_i)]

started_before_qp=unique(subset(reg_df_subset,post_t==0)$contract_award_unique_key)

m1=felm(wins_percentage_delay~treat_intensity_budget_i+
          treat_intensity_budget_i:post_t+
          post_t|
          0|
          0|
          contract_award_unique_key,
...
m2=felm(wins_percentage_delay~treat_intensity_budget_i+
          treat_intensity_budget_i:post_t+
          log(wins_project_quarter_stage)+
          log(1+winsorized_initial_duration_in_days_i)+
        #  log(1+winsorized_initial_budget_i)+
...
m3=felm(wins_percentage_delay~treat_intensity_budget_i+
          treat_intensity_budget_i:post_t+
          log(wins_project_quarter_stage)+
          log(1+winsorized_initial_duration_in_days_i)+
        #  log(1+winsorized_initial_budget_i)+
...
m4=felm(wins_percentage_delay~treat_intensity_budget_i+
          treat_intensity_budget_i:post_t+
          log(wins_project_quarter_stage)+
          log(1+winsorized_initial_duration_in_days_i)+
      #    log(1+winsorized_initial_budget_i)+
...
m5=felm(wins_percentage_delay~treat_intensity_budget_i+
          treat_intensity_budget_i:post_t+
          log(wins_project_quarter_stage)+
          log(1+winsorized_initial_duration_in_days_i)+
        #  log(1+winsorized_initial_budget_i)+
...
portfolio_did=stargazer(m1,m2,m3,m4,m5,
          ...
          dep.var.labels="$PercentDelay_{it}$  ",
          dep.var.caption = "",
          covariate.labels =c("$TreatIntensity_i$ (Budget)",
                               "$Post_t$",
                               "$TreatIntensity_i$ (Budget) $\\times Post_t$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
          omit.stat=c("f", "ser"),
          column.sep.width = "-2pt",
          add.lines = list(c("Duration, Bids",rep("No",1),rep("Yes",4)),
...
## DID on large projects

* Parallel trends does not hold for this so not evaluating.
* Indirect Treat = 1 if Treat_i = 0 and number of small projects > 0
* Indirect_treat_intensity = Indirect Treat x log(1+number of small projects)
* Subset of large projects only
...
portfolio_did=stargazer(m1,m2,m3,m4,m5,
          ...
          dep.var.labels="$PercentDelay_{it}$  ",
          dep.var.caption = "",
          covariate.labels =c("$Treat_i$ (All Small)",
                               "$PortfolioTreat_i$ (Small with Large)",
                               "$Post_t$",
                               "$Treat_i$ (All Small) $\\times Post_t$",
                               "$PortfolioTreat_i$ (Small with Large) $\\times Post_t$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
## DID on small projects (Treatment Intensity)

- Archived as not putting in paper

Consider three groups based on contractor's portfolio:

- Group 1: Small projects (with concurrent large projects)
- Group 2: Only small projects
- Group 3: Only large projects

Define the following variables:

* Treat_Intensity_i = Treat_i x log(1 + number of small projects)
* Treat_Intensity_i (Small with large) = I(Contractor holds large projects) x Treat_i x log(1 + number of small projects)
* Treat_Intensity_Large_i = Treat_i x log(1 + number of large projects)

```{r ti_2, warning=FALSE, echo=FALSE, eval=FALSE, results='asis'}

# Remove small projects whose contractors hold only small projects
# subset(reg_df_subset,!(treat_i==1 & !(recipient_duns%in%large_contractors)))

reg_df_subset[,treat_intensity:=treat_new*log(1+num_small_projects)]
reg_df_subset[,treat_intensity_small_with_large:=
                treat_small_with_large_new*log(1+num_small_projects)]

reg_df_subset[,treat_intensity_large:=treat_new*log(1+num_large_projects)]

m1=felm(wins_percentage_delay~treat_intensity+
          treat_intensity_small_with_large+
          treat_intensity_small_with_large:post_t+
          treat_intensity:post_t+
          treat_intensity_large+
          post_t:treat_intensity_large+
          post_t|
          0|
          0|
          contract_award_unique_key,
...
m2=felm(wins_percentage_delay~treat_intensity+
          treat_intensity_small_with_large+
          treat_intensity_small_with_large:post_t+
          treat_intensity:post_t+
          treat_intensity_large+
...
m3=felm(wins_percentage_delay~treat_intensity+
          treat_intensity_small_with_large+
          treat_intensity_small_with_large:post_t+
          treat_intensity:post_t+
          treat_intensity_large+
...
m4=felm(wins_percentage_delay~treat_intensity+
          treat_intensity_small_with_large+
          treat_intensity_small_with_large:post_t+
          treat_intensity:post_t+
          treat_intensity_large+
...
m5=felm(wins_percentage_delay~treat_intensity+
          treat_intensity_small_with_large+
          treat_intensity_small_with_large:post_t+
          treat_intensity:post_t+
          treat_intensity_large+
...
portfolio_did=stargazer(m1,m2,m3,m4,m5,
          ...
          dep.var.labels="$PercentDelay_{it}$  ",
          dep.var.caption = "",
          covariate.labels =c("$TreatIntensity_i$ (All Small)",
                               "$TreatIntensity_i$ (Small with Large)",
                               "$TreatIntensity_i$ (Large)",
                               "$Post_t$",
                               "$TreatIntensity_i$ (Small with Large) $\\times Post_t$",
                               "$TreatIntensity_i$ (All Small) $\\times Post_t$",
                               "$TreatIntensity_i$ (Large) $\\times Post_t$",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
