...
reg_df[,number_of_offers_received:=Winsorize(number_of_offers_received,
                                             probs=c(0.05,0.95),
                                             na.rm=T)]
...
```{r summary_stats_before_qp, eval=FALSE, include=FALSE}

treated_projs_preqp=subset(reg_df,treat_i==1 & post_t==0)
control_projs_preqp=subset(reg_df,treat_i==0 & post_t==0)

treated_projs_preqp[,percentage_delay:=100*percentage_delay]
control_projs_preqp[,percentage_delay:=100*percentage_delay]

#### Percentage delay #### 
## Treated projects
mean(subset(treated_projs_preqp,is.finite(percentage_delay)
            & percentage_delay<700)$percentage_delay,na.rm=T)

sd(subset(treated_projs_preqp,is.finite(percentage_delay)
            & percentage_delay<700)$percentage_delay,na.rm=T)

nrow(subset(treated_projs_preqp,!is.na(percentage_delay)))

quantile(treated_projs_preqp$percentage_delay,0.05,na.rm=T)
quantile(treated_projs_preqp$percentage_delay,0.95,na.rm=T)

## Control projects
mean(subset(control_projs_preqp,is.finite(percentage_delay)
            & percentage_delay<700)$percentage_delay,na.rm=T)

sd(subset(control_projs_preqp,is.finite(percentage_delay)
            & percentage_delay<700)$percentage_delay,na.rm=T)

nrow(subset(control_projs_preqp,!is.na(percentage_delay)))

quantile(control_projs_preqp$percentage_delay,0.05,na.rm=T)
quantile(control_projs_preqp$percentage_delay,0.95,na.rm=T)

#### Initial duration #### 
## Treated projects
mean(subset(treated_projs_preqp,
            !is.na(percentage_delay) & initial_duration_in_days_i>0 &
              initial_duration_in_days_i<=600 )$initial_duration_in_days_i,na.rm=T)

sd(subset(treated_projs_preqp,
            !is.na(percentage_delay) & initial_duration_in_days_i>0 &
              initial_duration_in_days_i<=600)$initial_duration_in_days_i,na.rm=T)

quantile(subset(treated_projs_preqp,initial_duration_in_days_i>0)$initial_duration_in_days_i,
         0.95,na.rm=T)
quantile(subset(treated_projs_preqp,initial_duration_in_days_i>0)$initial_duration_in_days_i,
         0.05,na.rm=T)
...
quantile(subset(control_projs_preqp,initial_duration_in_days_i>0)$initial_duration_in_days_i,
         0.95,na.rm=T)
quantile(subset(control_projs_preqp,initial_duration_in_days_i>0)$initial_duration_in_days_i,
         0.05,na.rm=T)
...
mean(subset(treated_projs_preqp,
            !is.na(percentage_delay) & base_and_all_options_value>0 &
              base_and_all_options_value<=7e+07 )$base_and_all_options_value,na.rm=T)

sd(subset(treated_projs_preqp,
            !is.na(percentage_delay) & base_and_all_options_value>0 &
              base_and_all_options_value<=7e+07 )$base_and_all_options_value,na.rm=T)

quantile(subset(treated_projs_preqp,base_and_all_options_value>0)$base_and_all_options_value,
         0.95,na.rm=T)
quantile(subset(treated_projs_preqp,base_and_all_options_value>0)$base_and_all_options_value,
         0.05,na.rm=T)
...
quantile(subset(control_projs_preqp,base_and_all_options_value>0)$base_and_all_options_value,
         0.95,na.rm=T)
quantile(subset(control_projs_preqp,base_and_all_options_value>0)$base_and_all_options_value,
         0.05,na.rm=T)
...
mean(subset(treated_projs_preqp,
            !is.na(percentage_delay))$number_of_offers_received_original,na.rm=T)
...
reg_df_first_reported[,number_of_offers_received:=Winsorize(number_of_offers_received,
                    probs=c(0.05,0.95),
                    na.rm=T)]
