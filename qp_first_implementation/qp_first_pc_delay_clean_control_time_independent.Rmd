...
contract_financing_contractor=reg_df_subset[,sum(contract_financing_i),
                                            by='recipient_duns']
...
reg_df_subset=merge(reg_df_subset,
                    contract_financing_contractor,
                    by='recipient_duns',
                    all.x=T)

reg_df_subset[,cf_contractor:=ifelse(num_projects_receiving_financing>0,1,0)]
...
baseline_reg=felm(wins_percentage_delay~treat_new+
                    post_t:treat_new+
                     post_t+
                    cf_contractor+
                    post_t:cf_contractor+
                    treat_new:cf_contractor+
                    post_t:treat_new:cf_contractor|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df_subset, 
...
```{r num_projects_log, results='asis',echo=FALSE}
# Baseline Regressions 
baseline_reg=felm(wins_percentage_delay~treat_new+
                    post_t:treat_new+
                     post_t+
                    log(1 + num_projects_receiving_financing)+
                    post_t:log(1 + num_projects_receiving_financing)+
                    treat_new:log(1 + num_projects_receiving_financing)+
                    post_t:treat_new:log(1 + num_projects_receiving_financing)|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df_subset, 
...
contract_financing_table=stargazer(baseline_reg,
          ...
          covariate.labels =c("$Treat_i$",
                               "$Post_t$",
                               "Num projects receiving CF (log)",
                               "$Treat_i \\times Post_t$",
                               "$Post_t \\times $ Num projects receiving CF (log)",
                               "$Treat_i \\times $ Num projects receiving CF (log)",
                               "$Treat_i \\times Post_t \\times $ Num projects receiving CF (log)",
                               "Constant"),
          object.names=FALSE, 
          model.numbers=TRUE,
          font.size = "small",
...
* CF = 1 if contractor is the upper half of those receiving financing on projects.
* That is, number of projects on which contractor receives financing is greater than the median of those receiving financing on projects.

```{r cf_contractor_level_median, results='asis',echo=FALSE}

contract_financing_contractor[,above_median_cf:=ifelse(num_projects_receiving_financing>
                                                      median(subset(contract_financing_contractor,num_projects_receiving_financing>0)$num_projects_receiving_financing),1,0)]

reg_df_subset=merge(reg_df_subset,
                    contract_financing_contractor[,c('recipient_duns','above_median_cf')],
                    by='recipient_duns',
                    all.x=T)
...
baseline_reg=felm(wins_percentage_delay~treat_new+
                    post_t:treat_new+
                     post_t+
                    above_median_cf+
                    post_t:above_median_cf+
                    treat_new:above_median_cf+
                    post_t:treat_new:above_median_cf|
                    0| # no fixed effects
                    0| # no IV
                    contract_award_unique_key, # clustered at project level
                  data=reg_df_subset, 
